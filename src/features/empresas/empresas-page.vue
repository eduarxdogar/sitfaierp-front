          ...
          <tbody class="divide-y divide-slate-100">
            <template v-for="empresa in empresas" :key="empresa.id">
              ...
              <tr class="hover:bg-slate-50 transition-colors group">
                ...
                <td class="py-2 px-3 font-semibold text-slate-800">{{ empresa.razonSocial }}</td>
                <td class="py-2 px-3 text-slate-500 font-mono">{{ empresa.ruc }}</td>
                <td class="py-2 px-3 text-center">
                  <span :class="{'bg-emerald-100 text-emerald-700': empresa.estado, 'bg-red-100 text-red-700': !empresa.estado}" class="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold">
